...
**AI / RAG.** `src/lib/ai/pipeline.ts` builds a system prompt from the hotel's profile, rooms (including photo URLs), FAQs, and offers, retrieves the most relevant knowledge-base chunks for the guest's message (`src/lib/ai/rag.ts`, pgvector cosine similarity), and generates a reply through a **7-layer fallback chain of providers** (`src/lib/ai/fallback-provider.ts`): Groq → Gemini → Cerebras → four curated OpenRouter free models → SambaNova → Mistral → Anthropic, tried in order — whichever have no API key configured fail instantly and the chain moves on, so there's no single point of failure on a free-tier rate limit. Groq leads for latency (live-tested under 500ms on its free tier). The order beneath Groq is deliberately *not* "however they were added" — it's ranked by actual free-tier depth, live-verified rather than assumed: Gemini's free tier (~1,000-1,500 requests/day, no card) is genuinely the most generous and has a fully independent quota, so it sits ahead of OpenRouter; OpenRouter's four curated free models (`src/lib/ai/openrouter-provider.ts`, a factory since it's instantiated once per model, env-overridable as a unit via `OPENROUTER_FREE_MODELS`) turned out to share **one 50-requests/day quota for the whole account**, not independent quotas per model — reproduced live when all four failed with the identical 429 in the same second — so they're now correctly understood as one shallow pool, not four deep ones. SambaNova's free tier is thinner still (20 requests/day). Anthropic is the final paid safety net, only reached if every free tier above is simultaneously exhausted — confirmed this can actually happen (Groq + OpenRouter both hit their caps at once during testing), which is why the chain now has this many layers instead of stopping at two. Every provider call is capped to a single attempt and a short timeout (5–15s) — the SDKs' own default retry/backoff behavior (Gemini: up to 5 attempts, Anthropic: 10-minute default timeout) was found to silently add up to a minute of latency per reply when left at defaults, fighting against this app's own fallback design.
...
**Conversation memory.** The AI pipeline only ever loads the last 12 messages of a conversation, so anything the guest said before that window is invisible to it. That was fine for the reply itself, but not for the facts a booking actually needs: the guest's party size used to be re-derived on *every* turn by pattern-matching the visible transcript, which meant that in any conversation longer than 12 messages, the turn where they answered "how many people?" scrolled out of context and Anushka asked them all over again — the single most-patched bug in the project's history, and one no amount of better pattern-matching can fix, because the text being matched against is gone. The captured slots are now stored on the contact instead (`Contact.pendingRoomId` / `pendingCheckIn` / `pendingCheckOut` / `pendingGuestCount`) the turn they're first stated, and read back from there. Guest count is captured from two sources: the stable row id of a GUEST_COUNT tap (resolved directly in `handle-inbound-message.ts`, so rewording a button title can't silently break capture) and a text extractor for typed messages covering English, Hinglish, and native Telugu phrasings (`src/lib/booking/guest-count.ts`). A later *explicit* count overwrites an earlier one — deliberately, since the "3+ people" row can only be stored as its floor of 3 — while a bare number is only ever read as a headcount in direct answer to the question, so a stray budget or room number can't overwrite it. All of it is cleared on booking completion, since `Contact` is a durable per-phone record a repeat guest reuses across stays.

**Availability.** Once the guest's dates are known, every room the hotel can't actually deliver is held out of the conversation *before* Anushka speaks — `findUnavailableRoomIds` resolves the taken set in one query (`src/lib/booking/availability.ts`) and the system prompt lists only bookable rooms, naming the rest under an explicit "already booked, do not recommend" heading so a guest asking for one by name gets a straight answer instead of silence. The same filter applies to both real-data room List Messages, and a room tap is re-checked at the moment it's tapped, since a list sent before the guest settled their dates stays tappable in their chat history forever. If nothing is free, Anushka says so and reopens the date picker rather than sending an empty list. Previously availability was consulted at exactly one moment — the "Confirm booking" tap — so a guest could be shown a room, sent its photos, and walked through agreeing dates for it, only for the clash to surface after they'd already committed.

**Tappable date picker (no Meta approval needed).** Guests pick a real check-in day from a list of actual dates, then the number of nights, entirely through ordinary WhatsApp List Messages (`src/lib/whatsapp/date-picker-message.ts`). Asking for nights rather than a second date grid makes an invalid range structurally impossible. This exists because publishing a Flow is gated behind Meta's per-number integrity check — verified live: create and upload both succeed, publish returns `139000/4233020 "Blocked by Integrity"` while the number's display name sits `DECLINED` — and that gate is per-hotel, so every tenant would hit it before its guests could pick a date. List Messages need no approval, so booking never depends on it. Reached via the "Pick exact dates" row (previously "I'll type dates", which fell through to the AI as free text and was a dead end for a guest who wanted a specific day).

**In-WhatsApp Flow calendar (optional, needs an approved display name).** Guests can pick their room and a real check-in/check-out date range on a native calendar inside WhatsApp and book in one tap, instead of answering questions one at a time — a published WhatsApp Flow (`src/lib/whatsapp/flows/`), sent on the "Book a room" tap. Publish it from **Settings → WhatsApp → Booking calendar**; the room list is a snapshot baked in at publish time (it's a static Flow with no hosted data endpoint), so re-publish after changing rooms or rates. Availability is still re-checked server-side on submit, so the one-tap route can't overbook. The Flow JSON and the send path both existed for a while but `HotelProfile.whatsappBookingFlowId` was read in two places and written in none — no publish path existed, so the id was always null and the calendar was unreachable code that no guest could ever see.

**Occupancy calendar.** `/calendar` shows every room against every night for the next four weeks, built from the same half-open overlap rule (`checkIn < otherCheckOut && checkOut > otherCheckIn`) the availability check uses — so the grid and what Anushka tells a guest can never disagree. The check-out day is shown free, because it is.

**Conversation soak testing.** `npm run soak -- 100000` drives that many randomly-ordered conversations through the real state machine and asserts invariants that must hold on every path: never re-ask a slot that's already settled, never repeat a question more than twice, never leave the guest with an empty reply, never swallow a photo request or a cancellation into the booking funnel, never claim a booking is confirmed without the tap. Every bug in this project's history was found the same way — a guest said something in an order nobody had written a test for — and example-based tests structurally cannot find those. Its first run at 5,000 conversations found a ~10% deadlock rate; at 100,000 it found three more distinct bugs. A `vitest`-resident 5,000-conversation version runs as part of `npm test`. Failures print the transcript and the seed, so they replay deterministically.
...
5. **AI** — set at least one of `OPENROUTER_API_KEY`, `GEMINI_API_KEY`, `GROQ_API_KEY`, `MISTRAL_API_KEY`, `ANTHROPIC_API_KEY` (chat — tried in that order, see Architecture above) and `VOYAGE_API_KEY` or `GEMINI_API_KEY` (embeddings).
...
- **Multi-provider AI fallback** (OpenRouter free models/Gemini/Groq/Mistral/Anthropic), not a single hardcoded provider — free tiers first, Anthropic as the paid/quality fallback if ever configured; swappable behind `AIProvider` (`src/lib/ai/provider.ts`).
...
src/
  ...
  lib/
    ai/                  AIProvider fallback chain (OpenRouter free models/Gemini/Groq/Mistral/Anthropic) + EmbeddingProvider

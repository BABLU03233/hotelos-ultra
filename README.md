...
**AI / RAG.** `src/lib/ai/pipeline.ts` builds a system prompt from the hotel's profile, rooms (including photo URLs), FAQs, and offers, retrieves the most relevant knowledge-base chunks for the guest's message (`src/lib/ai/rag.ts`, pgvector cosine similarity), and generates a reply through a **fallback chain of providers** (`src/lib/ai/fallback-provider.ts`): Groq → six curated OpenRouter free models → Gemini → Mistral → Anthropic, tried in order — whichever have no API key configured fail instantly and the chain moves on, so there's no single point of failure on a free-tier rate limit. Groq leads for latency (live-tested under 500ms on its free tier); once its daily free-tier cap is hit, the chain falls through the OpenRouter models, ordered smallest/fastest-first so a "guest expects an instant reply" goal survives even a bad run. The OpenRouter entries (`src/lib/ai/openrouter-provider.ts`, a factory since it's instantiated once per model) are live-verified against OpenRouter's current model list (`GET /api/v1/models`, filtered to `pricing.prompt === "0"`) rather than assumed — free-tier availability shifts as models get deprecated or added — and the whole list is env-overridable as a unit (comma-separated `OPENROUTER_FREE_MODELS`) if a chosen model gets deprecated or rate-limited harder. Anthropic is the final paid safety net, only reached if every free tier is simultaneously exhausted. Every provider call is capped to a single attempt and a short timeout (5–15s) — the SDKs' own default retry/backoff behavior (Gemini: up to 5 attempts, Anthropic: 10-minute default timeout) was found to silently add up to a minute of latency per reply when left at defaults, fighting against this app's own fallback design.
